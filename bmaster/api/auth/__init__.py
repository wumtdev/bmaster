...
from bmaster.api.auth.users import User, Account, UserLocalInfo
...
def jwt_encode(data: Any) -> str:
    return jwt.encode(data, key=config.jwt.secret_key, algorithm=config.jwt.algorithm)


def jwt_decode(token: str):
    return jwt.decode(
        jwt=token, key=config.jwt.secret_key, algorithms=[config.jwt.algorithm]
    )
...
class AuthToken(BaseModel):
    ...
    def validate_type(cls, data: Any, handler: ModelWrapValidatorHandler[Self]) -> Self:
        ent = handler(data)
        if cls is not AuthToken:
            return ent
        ent_type = ent.type
        ent_class = auth_token_registry.get(ent_type, None)
        if not ent_class:
            raise ValidationError(f"Unknown entity type: {ent_type}")
...
def require_auth_token(jwt_data: Annotated[Any, Depends(require_bearer_jwt)]):
    try:
        ...
        if (token.type == "root") != config.service.enabled:
            raise HTTPException(
                status.HTTP_401_UNAUTHORIZED,
                "bmaster.auth.service",
                headers={"WWW-Authenticate": "Bearer"},
            )
    except ValidationError:
        raise HTTPException(
            status.HTTP_401_UNAUTHORIZED,
            "bmaster.auth.invalid_token",
            headers={"WWW-Authenticate": "Bearer"},
        )
    return token
...
async def require_user(
    auth_token: Annotated[AuthToken, Depends(require_auth_token)],
) -> User:
...
def require_permissions(*permissions: str):
    def _check(user: Annotated[User, Depends(require_user)]):
        if not user.has_permissions(*permissions):
            raise HTTPException(
                status.HTTP_403_FORBIDDEN, "bmaster.auth.missing_permissions"
            )
...
async def authenticate_user(username: str, password: str) -> Optional[Account]:
    ...
    async with LocalSession() as session, session.begin():
        user = (
            (await session.execute(select(Account).where(Account.name == username)))
            .unique()
            .scalar_one_or_none()
        )
        if not user:
...
async def login(req: LoginRequest) -> Token:
    ...
    if not service_config.enabled:
        if username == "root":
            raise HTTPException(
                status.HTTP_401_UNAUTHORIZED,
                "bmaster.auth.login.invalid_credentials",
                headers={"WWW-Authenticate": "Bearer"},
            )
        ...
        auth_token = UserToken(user_id=user.id)
        return Token(
            access_token=jwt_encode(auth_token.model_dump()), token_type="bearer"
        )
    else:
        if username != "root":
            raise HTTPException(
